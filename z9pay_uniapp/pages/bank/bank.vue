<template>
	<view class="content">
		<z-paging ref="paging" v-model="lists" @query="fetchData">
			<view slot="top">
				<uni-nav-bar left-icon="arrow-left" title="收款卡管理" backgroundColor="#121212" :border="false" color="#edd185" height="96rpx" class="header" @clickLeft="$back()" left-width="96rpx" right-width="96rpx">
				...
				<view class="searchs">
					...
					<view class="more" v-if="lists.length > 0 && hasMoreAction" v-on:click="$refs.moreMenu.open()">
						<view class="text"> 关闭删除 </view>
					</view>
				</view>
				</view>
			...
			<view class="main">
				<view class="lists">
					<view class="item" v-bind:class="{ active: vo.show }" v-for="(vo, index) in lists" v-bind:key="index">
						...
						<view class="top-info" v-on:click="open(index)">
							<view class="top-header">
								...
								<view class="top-right-info">
									<view class="top-right-main">
										<view class="status" v-if="vo.collection_status == 1">
											{{ vo.collection_status_text }}
										</view>
										<view class="status forbid" v-else>
											{{ vo.collection_status_text }}
										</view>
									</view>
		...
		<uni-popup ref="addMenu" type="top" maskBackgroundColor="transparent" :animation="false">
			<view class="addMenuContent">
				<view class="item" v-on:click="openAddForm(vo.id)" v-for="(vo, index) in accountTypeList" v-bind:key="index">
					新增{{ vo.name }} </view>
			</view>
		</uni-popup>

		<uni-popup ref="moreMenu" type="top" maskBackgroundColor="transparent" :animation="false">
			<view class="moreMenuContent">
				<view class="item" v-bind:class="{ disabled: !hasOpenBank }" v-on:click="closeAll">
					一键关闭
				</view>
				<view class="item danger" v-if="$store.state.action_delete == 1" v-on:click="delAll">
					批量删除
				</view>
			</view>
		</uni-popup>
					...
					<view class="password">
						...
						<scroll-view scroll-y="true" class="scroll-view-forms">
							<view class="forms">
								...
								<view class="item" v-if="form1.account_type == 2 || form1.account_type == 3 || form1.account_type == 4 || form1.account_type == 5   || form1.account_type == 28">
...
<script>
	...
	export default {
		...
		computed: {
			...
			hasOpenBank() {
				return this.lists.some((item) => item.collection_status == 1);
			},
			hasMoreAction() {
				return this.hasOpenBank || this.$store.state.action_delete == 1;
			},
		},
		methods: {
			getCode(url) {
			...
			afterRead2(event) {
				...
				delBank(id, index) {
				this.$confirm("确定删除", () => {
					this.$ajax.delete("v2/user-banks/" + id).then((res) => {
						if (res) {
			...
			delAll() {
				this.$refs.moreMenu && this.$refs.moreMenu.close();
				this.$confirm("确定删除所有收款卡", () => {
					this.$ajax.get("v2/user-banks/clearBank").then((res) => {
						if (res) {
							this.lists = [];
						}
					});
				...
				this.$refs.moreMenu && this.$refs.moreMenu.close();
				if (!this.hasOpenBank) {
					this.$message("暂无开启中的收款卡");
					return;
				}
				this.$confirm("确定关闭所有开启中的收款卡？关闭后将不再参与收单", () => {
					uni.showLoading({
						title: "关闭中",
						mask: true,
					});
					this.$ajax.get("v2/user-banks/closeAll").then((res) => {
						uni.hideLoading();
						if (res) {
							if (this.form.collection_status == 1) {
								this.$refs.paging.reload();
							} else {
								this.lists.map((item, index) => {
									if (item.collection_status == 1) {
										this.$set(this.lists[index], "collection_status_text", "收单关闭");
										this.$set(this.lists[index], "collection_status", 0);
									}
								});
							}
							this.$message("已关闭开启中的收款卡");
						}
					}).catch(() => {
						uni.hideLoading();
						this.$message("关闭失败，请稍后重试");
					});
				});
			},
			fetchData(pageNo, pageSize) {
				this.$ajax.get("v2/user-banks",
					Object.assign({
							page: pageNo,
						},
						this.form
			...
			submitData() {
				if (this.form1.id > 0) {
					this.$ajax.put("v2/user-banks/" + this.form1.id, this.form1).then((res) => {
						if (res) {
							this.form1 = {
								id: 0,
								name: "",
					...
					this.$ajax.post("v2/user-banks", this.form1).then((res) => {
						if (res) {
							this.form1 = {
								id: 0,
								name: "",
			...
			setStatus(item, index) {
				this.$ajax.get("v2/user-banks/setStatus/" + item.id).then((res) => {
					if (res) {
...
<style lang="less" scoped>
	.content {
		...
		@media screen and (min-width: 751px) {
			.moreMenuContent {
				margin-right: calc((100vw - 750px) / 2 + 20px);
			}
		}
		/* #endif */

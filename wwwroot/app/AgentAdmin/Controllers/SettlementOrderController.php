...
use App\AgentAdmin\Actions\SettlementOrder\ExportData;
use App\Admin\Metrics\AgentAdmin\SettlementOrder\Card1;
use App\Admin\Metrics\AgentAdmin\SettlementOrder\Card2;
use App\Admin\Metrics\AgentAdmin\SettlementOrder\Card3;
use App\Services\Cache\Merchant\GetMerchantListInfoService;

class SettlementOrderController extends CommonController
{
    protected $translation = 'merchant-admin-settlement-order';
    ...
    protected function grid(): Grid
    {
        return Grid::make(TransferOrder::with(['merchant_info' => function ($query) {
            ...
            // 代理端结算订单只读取列表展示字段，减少大表返回数据量。
            $grid->model()->where('type', 1)->whereIn('merchant_agent1_id', $childAgentIds)->select([

...
class CurrencyReportRepairCommand extends Command
{
    protected $signature = 'report:repair-currency {currency_id=0 : 币种ID} {date=0 : 统计日期，格式 YYYY-MM-DD}';

    protected $description = '修复指定币种指定日期的代收、代付报表统计';

    public function handle(): int
    {
        $currencyId = $this->positiveIntegerArgument('currency_id', '币种ID');
        ...
        if (empty(collect(config('default.currency'))->firstWhere('id', $currencyId))) {
            $this->error('币种不存在');
            return 1;
        }

        App::make(ReportPendingDateService::class)->addDates([$date]);
        $this->info("币种报表已加入待重建队列，币种ID：{$currencyId}，日期：{$date}");

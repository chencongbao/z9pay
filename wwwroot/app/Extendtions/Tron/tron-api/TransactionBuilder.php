...
class TransactionBuilder
{
    ...
    */
     ...
     */
    public function sendTrx($to, $amount, string $from = null,$Permission_id = null)
    {
        ...
        $response = $this->tron->getManager()->request('wallet/createtransaction', $sendData);

        return $response;
    }
    public function delegateResource($to, $amount,string $resource = 'ENERGY',  $lock=false, $lock_period=0, string $from = null,$Permission_id = null)
    {
        if (!is_float($amount) || $amount < 0) {
            throw new TronException('Invalid amount provided');
        }
    ...
    public function undelegateResource($to, $amount,string $resource = 'ENERGY', string $from = null,$Permission_id = null)
    {
        if (!is_float($amount) || $amount < 0) {
            throw new TronException('Invalid amount provided');
        }
    ...
    public function freezeBalanceV2(float $amount = 0, string $resource = 'BANDWIDTH', string $address = "",$Permission_id = null)
    {
        if (!in_array($resource, ['BANDWIDTH', 'ENERGY'])) {
            throw new TronException('Invalid resource provided: Expected "BANDWIDTH" or "ENERGY"');
        }
        ...
        return $this->tron->getManager()->request('wallet/freezebalancev2', $sendData);
    }
    /**
     ...
     * @throws TronException
     */
    public function unfreezeBalance(string $resource = 'BANDWIDTH', string $owner_address = "",$Permission_id = null)
    {
        if (!in_array($resource, ['BANDWIDTH', 'ENERGY'])) {
            throw new TronException('Invalid resource provided: Expected "BANDWIDTH" or "ENERGY"');
        }
        ...
        return $this->tron->getManager()->request('wallet/unfreezebalance', $sendData);
    }
     ...
     * @throws TronException
     */
    public function updateToken(string $description = null, string $url = null, int $freeBandwidth = 0, int $freeBandwidthLimit = 0, $address = null)
    {

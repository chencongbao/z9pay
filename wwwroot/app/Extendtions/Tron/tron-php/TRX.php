...
class TRX implements WalletInterface
{
    ...
    {
        $this->_api = $_api;
        $host = config('default.base_uri');
        $fullNode = new HttpProvider($host);
        $solidityNode = new HttpProvider($host);
        $eventServer = new HttpProvider($host);
        $signServer = new HttpProvider($host);
        $explorer = new HttpProvider(config("default.explorer_uri"));
        try {
            $this->tron = new Tron($fullNode, $solidityNode, $eventServer,$signServer,$explorer);

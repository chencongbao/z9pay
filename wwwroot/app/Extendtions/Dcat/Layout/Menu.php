...
namespace App\Extendtions\Dcat\Layout;
...
class Menu
{
    protected static $helperNodes = [
        ...
        [
            'id'        => 9,
            'title'     => 'admin_development_merchant',
            'icon'      => 'fa fa-drivers-license',
            'uri'       => '',
            'parent_id' => 1,
        ],
        ...
        [
            ...
            'title'     => 'permission',
            'icon'      => '',
            'uri'       => 'merchant/auth/permissions',
            'parent_id' => 9,
        ],
        [
            'id'        => 13,
            'title'     => 'admin_development_agent',
            'icon'      => 'fa fa-drivers-license',
            'uri'       => '',
            'parent_id' => 1,
        ],
    ...
    public function register()
    {
        if (! admin_has_default_section(Admin::SECTION['LEFT_SIDEBAR_MENU'])) {
            admin_inject_default_section(Admin::SECTION['LEFT_SIDEBAR_MENU'], function () {
                ...
                return $this->toHtml($this->prependSystemHomeNode((new $menuModel())->allNodes()->toArray()));
            });
        }
            ...
            if(Admin::guard()->check()){
                if(Admin::user()->isAdministrator()){
                    if(config('app.debug')){
                        ...
                        $menus = collect(static::$helperNodes)->filter(function ($value){
    ...
    /**
     * 系统首页是固定入口，不依赖数据库菜单，避免 iframe-tab 关闭首页后无法再次进入。
     */
    protected function prependSystemHomeNode(array $nodes): array
    {
        if (Admin::app()->getName() !== 'admin') {
            return $nodes;
        }

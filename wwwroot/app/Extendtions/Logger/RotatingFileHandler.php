...
class RotatingFileHandler extends StreamHandler
{
    ...
    protected function rotate(): void
    {
        ...
        $logFiles = $this->getGlobPattern();
        if (false === $logFiles) {
            // failed to glob
            return;
        }

        if ($this->maxFiles >= count($this->getGlobPattern())) {
            // no files to remove
            return;
        }
        ...
        foreach (array_slice($logFiles, $this->maxFiles) as $file) {
            if (is_writable($file)) {
                ...
                unlink($file);
                File::deleteDirectory(storage_path('logs' . DIRECTORY_SEPARATOR . $names[count($names)-3]));
    ...
    protected function getGlobPattern(): Array
    {
        $data = [];
        $names = explode(DIRECTORY_SEPARATOR,$this->filename);
        $dir = scandir(storage_path('logs'));
        if (!empty($dir)) {
            foreach ($dir as $folder) {
                if (is_dir(storage_path('logs' . DIRECTORY_SEPARATOR . $folder)) && $folder != '.' && $folder != '..' && $this->isDateTime($folder)) {
                    $data[]  = storage_path('logs' . DIRECTORY_SEPARATOR . $folder.DIRECTORY_SEPARATOR.$names[count($names)-2].DIRECTORY_SEPARATOR."debug-".$folder.".log");
                }
            }
        }
       return $data;

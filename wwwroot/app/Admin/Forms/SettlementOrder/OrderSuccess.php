...
class OrderSuccess extends Form implements LazyRenderable
{
    ...
    public function handle(array $input)
    {
        ...
        try {
            ...
            // 统一走结算成功服务，状态、余额、佣金、缓存由服务内部处理。
            $order = app(SettlementOrderSuccessService::class)->excute($order->id, $actualAmount, $remark, $admin->id, 1);
            app(CreateTransferOrderLogService::class)->excute($order->id, '订单手动成功', '操作员：' . $admin->username);

            app(SystemLogService::class)->logAction(
                actionKey: 'settlement.order.success',
                text: '结算订单手动成功',
                subject: $order,
                properties: [
                    'order_id' => $order->id,
    ...
    {
        return Admin::user()->can('settlement-order-manual-success');
    }

    public function form()
    {
        $this->confirm('提示', '成功确认?');

...
class BatchQueryTransferOrderForm extends Form implements LazyRenderable
{
    use LazyWidget;
    ...
    public function handle(array $input)
    {
        ...
        if ($orderIds->isEmpty()) {
            return $this->response()->warning('没有符合条件的代付订单');
        }

        $total = $orderIds->count();
        foreach ($orderIds as $orderId) {
            dispatch(new HandlePendingQueryTransferOrderResultJob($orderId))->onQueue('query');
        }

...
class ReduceBalance extends Form implements LazyRenderable
{
    ...
    public function handle(array $input)
    {
        try {
            ...
            DB::transaction(function () use ($adminUser, $agentId, $amount, $remark) {
                ...
                $desc = sprintf('手动减项 商户代理余额 %.2f', $amount);
                app(SystemLogService::class)->logAction(
                    actionKey: 'agent.balance.reduce',
                    text: '手动减项 商户代理余额',
                    subject: $agent,
                    properties: [
    ...
    {
        return Admin::user()->can('merchant-agent-balance-log-reduce');
    }

    public function form()
    {
        $this->display('name', '代理');
        $this->text('amount', '减项金额')->rules(['numeric', 'min:0.01', 'max:9999999', new DecimalTwoPlaces()], ['numeric' => '减项金额不合法', 'min' => '减项金额不能小于0.01', 'max' => '减项金额不合法'])->required();

...
class UpdatePayMinMaxAmountForm extends Form implements LazyRenderable
{
    use LazyWidget;
    ...
    public function handle(array $input)
    {
        try {
            ...
            $minAmount = (float) bob_amount_format($input['pay_min_amount'] ?? 0);
            $maxAmount = (float) bob_amount_format($input['pay_max_amount'] ?? 0);
            ...
            $changes = [
                'pay_min_amount' => bob_amount_format($minAmount),
                'pay_max_amount' => bob_amount_format($maxAmount),
            ];
            ...
            $result = DB::transaction(function () use ($keys, $changes, $admin) {
                ...
                app(SystemLogService::class)->logAction(
                    actionKey: 'merchant.channel.batch_update_pay_limit',
                    text: '批量修改 代收单笔限额',
                    subject: null,
                    properties: [
                        'ids' => $keys,
    ...
    {
        return Admin::user()->can('merchant-channel-batch-pay-limit');
    }

    public function form()
    {
        $this->number('pay_min_amount', '代收单笔下限')->rules(['numeric', 'between:0,999999999', new DecimalTwoPlaces()], ['numeric' => '数值不合法', 'between' => '数值不合法'])->default(0)->required();
        $this->number('pay_max_amount', '代收单笔上限')->rules(['numeric', 'between:0,999999999', new DecimalTwoPlaces(), 'gte:pay_min_amount'], ['numeric' => '数值不合法', 'between' => '数值不合法', 'gte' => '代收单笔上限必须大于等于代收单笔下限'])->default(0)->required();

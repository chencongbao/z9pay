...
class AddYajinBalance extends Form implements LazyRenderable
{
    use LazyWidget;
    ...
    public function handle(array $input)
    {
        try {
            ...
            $handledUserId = DB::transaction(function () use ($admin, $userId, $amount, $remark) {
                ...
                $desc = sprintf('手动增项 金主保证金 %.2f', $amount);
                app(SystemLogService::class)->logAction(
                    actionKey: 'user.deposit_amount.add',
                    text: '手动增项 金主保证金',
                    subject: $user,
                    properties: [
                        'user_id' => $user->id,
    ...
    {
        return Admin::user()->can('user-deposit-balance-add');
    }

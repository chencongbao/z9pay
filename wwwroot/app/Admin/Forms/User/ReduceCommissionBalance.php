...
class ReduceCommissionBalance extends Form implements LazyRenderable
{
    use LazyWidget;
    ...
    public function handle(array $input)
    {
        try {
            ...
            DB::transaction(function () use ($admin, $userId, $amount, $remark) {
                $user = User::query()->whereKey($userId)->lockForUpdate()->first(['id', 'name', 'commission_balance_amount']);
                if (!$user) {
                    throw new \Exception('非法操作');
                }
                if ($amount > (float)$user->commission_balance_amount) {
                    throw new \Exception('减项金额不能大于佣金余额');
                }

                // 调用统一余额服务扣减金主佣金，并写入操作日志。
                ...
                $desc = sprintf('手动减项 金主佣金 %.2f', $amount);
                app(SystemLogService::class)->logAction(
                    actionKey: 'user.commission.reduce',
                    text: '手动减项 金主佣金',
                    subject: $user,
                    properties: [
                        'user_id' => $user->id,
    ...
    {
        return Admin::user()->can('user-commission-balance-reduce');
    }
    ...
    {
        $userId = (int)($this->payload['id'] ?? 0);
        $user = User::query()->whereKey($userId)->first(['id', 'username', 'name', 'commission_balance_amount']);

        return [
            'name' => optional($user)->bname,
            'amount' => floatval(optional($user)->commission_balance_amount),
            'remark' => '',
        ];
    }
}

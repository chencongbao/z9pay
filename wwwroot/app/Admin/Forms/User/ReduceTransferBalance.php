...
class ReduceTransferBalance extends Form implements LazyRenderable
{
    use LazyWidget;
    ...
    public function handle(array $input)
    {
        try {
            ...
            DB::transaction(function () use ($admin, $userId, $amount, $remark) {
                ...
                $desc = sprintf('手动减项 金主代付账户 %.2f', $amount);
                app(SystemLogService::class)->logAction(
                    actionKey: 'user.transfer.reduce',
                    text: '手动减项 金主代付账户',
                    subject: $user,
                    properties: [
                        'user_id' => $user->id,
    ...
    {
        return Admin::user()->can('user-transfer-balance-reduce');
    }
    ...
    {
        $userId = (int)($this->payload['id'] ?? 0);
        $user = User::query()->whereKey($userId)->first(['id', 'username', 'name', 'transfer_balance_amount']);

        return [
            'name' => optional($user)->bname,
            'amount' => floatval(optional($user)->transfer_balance_amount),
            'remark' => '',
        ];
    }
}

...
class OrderTimeout extends Form implements LazyRenderable
{
    use LazyWidget;
    ...
    public function handle(array $input)
    {
        try {
            $admin = Admin::user();
            if ($admin->cannot('deposit-order-manual-timeout')) {
                throw new RuntimeException('无手动超时代收订单权限');
            }

            $id = intval($this->payload['id'] ?? 0);
            $remark = trim((string) ($input['remark'] ?? '')) ?: '订单手动超时';
            ...
            $order = DB::transaction(function () use ($id, $remark, $admin) {
                ...
                // 手动超时统一走状态服务，状态落库后立即刷新订单缓存。
                App::make(DepositOrderStatusService::class)->markTimeout($order, $remark);

                if ($order->user_id > 0) {
                    App::make(GetUserDaifukuanDepositOrderListService::class)->remove($order->user_id, $order);
                }

                App::make(CreateDepositOrderLogService::class)->excute($order->id, '订单手动超时', '操作员：' . $admin->username);

                return $order;
            });

            app(SystemLogService::class)->logAction(
                actionKey: 'deposit.order.timeout',
                text: '代收订单手动超时',
                subject: $order,
                properties: [
                    'order_id' => $order->id,
    ...
    {
        $id = intval($this->payload['id'] ?? 0);
        $order = $id > 0 ? DepositOrder::query()->whereKey($id)->first(['id', 'order_no', 'ordernumber', 'amount']) : null;

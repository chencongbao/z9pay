...
use App\Jobs\BatchMerchantTransferCallbackJob;
...
class BatchCallbackForm extends Form implements LazyRenderable
{
    ...
    {
        try {
            if (Admin::user()->cannot('transfer-order-batch-callback')) {
                throw new \Exception('无批量回调代付订单权限');
            }
            ...
            dispatch(new BatchMerchantTransferCallbackJob($merchantUserId, $dateAdd, $limit))->onQueue('callback_low');
    ...
    {
        return Admin::user()->can('transfer-order-batch-callback');
    }
    ...
    {
        return max(1, min((int)($input['limit'] ?? self::DEFAULT_LIMIT), self::MAX_LIMIT));
    }
    ...
    {
        return 'admin_transfer_batch_callback:' . ($merchantUserId ?: 'all') . ':' . $dateAdd;
    }
}

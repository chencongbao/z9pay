...
use App\Admin\Renderable\ReportCurrency\SummaryCard;
use App\Services\Cache\Merchant\CacheMerchantBaseInfoService;

class ReportCurrencyController extends CommonController
{
    public $title = '币种报表';
    ...
    protected function grid(): Grid
    {
        ...
        $isAllCurrency = request()->has('cid') && (int) request('cid') <= 0;
        if ($isAllCurrency) {
        ...
        return Grid::make($query, function (Grid $grid) use ($mid, $sourceId, $currencyId, $currencyList, $currencyOptions, $currentCurrency, $merchantBaseInfoService) {
            ...
            $grid->header(function () use ($mid, $sourceId, $currencyId) {
                $tab = Tab::make();
                $tab->addLink('代收', admin_route('report-currencys.index', $this->tabQuery(1, $currencyId, $mid)), $sourceId === 1);
                $tab->addLink('代付', admin_route('report-currencys.index', $this->tabQuery(2, $currencyId, $mid)), $sourceId === 2);
                $tab->addLink('结算', admin_route('report-currencys.index', $this->tabQuery(3, $currencyId, $mid)), $sourceId === 3);
            ...
            $grid->wrap(function (Renderable $view) use ($currencyId, $currencyList) {
                ...
                $row->column(2, function (Column $column) use ($currencyId, $currencyList) {
                    $left = new LeftSide();
                    $left->title('币种列表')->field('cid')->default($currencyId)->prependAll('全部币种', true)->data($currencyList->toArray());
                    $column->row($left);
                });

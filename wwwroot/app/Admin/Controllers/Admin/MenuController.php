...
class MenuController extends AdminController
{
        ...
        return $content
            ...
            ->body(function (Row $row) {
                ...
                $row->column(5, function (Column $column) {
                    ...
                    $form->select('parent_id', trans('admin.parent_id'))->options(self::translatedSelectOptions($menuModel::selectOptions()));
    ...
    protected function treeView()
    {
        ...
        return new Tree(new $menuModel(), function (Tree $tree) {
            ...
            $tree->branch(function ($branch) {
                $title = self::translateTitle($branch['title']);
    ...
    public function form()
    {
        ...
        return Form::make(Menu::with($relations), function (Form $form) use ($menuModel) {
            ...
            $form->select('parent_id', trans('admin.parent_id'))->options(function () use ($menuModel) {
                return self::translatedSelectOptions($menuModel::selectOptions());
            })->saving(function ($v) {
                return (int) $v;
            });
            $form->text('title', trans('admin.title'))->required();
    ...
    protected static function translatedSelectOptions(array $options): array
    {
        foreach ($options as $key => $title) {
            $options[$key] = self::translateOptionTitle($title);
        }

        return $options;
    }

    protected static function translateOptionTitle($title): string
    {
        $title = (string) $title;
        $parts = explode('&nbsp;', $title);
        $rawTitle = array_pop($parts);
        $translatedTitle = self::translateTitle($rawTitle);

        return $parts ? implode('&nbsp;', $parts) . '&nbsp;' . $translatedTitle : $translatedTitle;
    }

    protected static function translateTitle($title): string
    {
        $title = trim((string) $title);
        $key = 'menu.titles.' . trim(str_replace(' ', '_', strtolower($title)));

        return Lang::has($key) ? __($key) : $title;
    }
}

...
use App\Repositories\MerchantPermission;
...
class PermissionController extends AdminController
{
    ...
    protected function treeView()
    {
        ...
        $routePrefix = (string) config('merchant-admin.route.prefix');
        ...
        return new Tree(new $permissionModel(), function (Tree $tree) use ($routePrefix, $color) {
            ...
            $tree->branch(function ($branch) use ($routePrefix, $color) {
                $branchName = htmlspecialchars($this->translatePermissionName((string) ($branch['name'] ?? '')), ENT_QUOTES, 'UTF-8');
                ...
                $pathHtml = collect($paths)->map(function ($path) use (&$methods, $routePrefix, $color) {
                    ...
                    if ($path !== '...' && $routePrefix !== '' && ! Str::contains($path, '.')) {
                        $path = trim(bob_admin_base_path($path, 'merchant-admin'), '/');
                    }
    ...
    {
        $with = [];
        $bindMenu = config('merchant-admin.menu.permission_bind_menu', true);

        if ($bindMenu) {
            $with[] = 'menus';
        }

        return Form::make(MerchantPermission::with($with), function (Form $form) use ($bindMenu) {
            $permissionTable = config('merchant-admin.database.permissions_table');
    ...
    {
        $prefix = (string) config('merchant-admin.route.prefix');
    ...
    private function translatePermissionName(string $name): string
    {
        $key = 'menu.titles.' . $name;

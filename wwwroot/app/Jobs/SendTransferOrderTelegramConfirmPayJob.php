...
class SendTransferOrderTelegramConfirmPayJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            if (empty($order->channel_info)) {
                throw new \Exception('支付渠道未获取到');
            }
            $channel_info = $order->channel_info;
            if (empty($channel_info['channel_id']) || empty($channel_info['classname'])) {
                throw new \Exception('支付渠道信息不完整');
            }
            $channel = Channel::where('id', $channel_info['channel_id'])->first(['id', 'status', 'name']);
            if (!$channel || (int)$channel->status !== 1) {
                throw new \Exception('渠道不可用或已关闭');
            }
            $operatorName = $this->operatorName();
            $order->transfer_order_confirm_remark = [
                "remark" => "确认人：" . $operatorName . "，发起代付",
            ...
            $createTransferOrderLogService->excute($order->id, '商户群确认代付', [
                '确认人' => $operatorName,
                '确认动作' => '发起代付',
                '通道ID' => $channel_info['channel_id'] ?? '',
                '商户通道ID' => $channel_info['merchant_channel_id'] ?? '',
                '通道名称' => $channel_info['name'] ?? ($channel->name ?? ''),
                '选中代付额外手续费' => $channel_info['merchant_extra_fee'] ?? 0,
            ]);
            dispatch(new SendTransferOrderPaymentJob($order, $channel_info))->onQueue('transfer')->afterCommit();
            DB::commit();
            ...
            if ($createTransferOrderLogService && $order) {
                $createTransferOrderLogService->excute($order->id, '商户群确认代付失败', [
                    '失败原因' => $e->getMessage(),
                    '当前状态' => $order->status ?? '',
            ...
            App::make(ReportExceptionService::class)->report('商户群确认代付失败', $e, [

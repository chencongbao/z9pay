...
class UserTodayDepositStatsService
{
    public function increase(int $userId, float $amount = 0, int $number = 0, float $income = 0): void
    {
        ...
        app(UserTodayStatsRebuildService::class)->runWithRebuildLock(function () use ($userId, $today, $number, $amount, $income) {
            // 成功代收时直接维护金主/代理今日统计，避免后台弹窗读取 Redis 或实时聚合订单。
            User::query()->whereKey($userId)->update([
                'today_deposit_total_number' => DB::raw("IF(today_deposit_stat_date = '{$today}', today_deposit_total_number + {$number}, {$number})"),
                'today_deposit_total_amount' => DB::raw("IF(today_deposit_stat_date = '{$today}', today_deposit_total_amount + {$amount}, {$amount})"),
                'today_deposit_total_income' => DB::raw("IF(today_deposit_stat_date = '{$today}', today_deposit_total_income + {$income}, {$income})"),
    ...
    public function amount($user): float
    {
        ...
        return bob_amount_format($user->today_deposit_total_amount ?? 0);
    }
    ...
    public function number($user): int
    {
        ...
        return intval($user->today_deposit_total_number ?? 0);
    }
    ...
    public function income($user): float
    {
        ...
        return bob_amount_format($user->today_deposit_total_income ?? 0);
    }

    public function findAmount(int $userId): float
    {
    ...
    public function findNumber(int $userId): int
    {
        $user = $this->findStats($userId);
    ...
    public function findIncome(int $userId): float
    {
        $user = $this->findStats($userId);
    ...
    private function findStats(int $userId): ?User
    {
        ...
        return User::query()
            ->select(['id', 'today_deposit_stat_date', 'today_deposit_total_number', 'today_deposit_total_amount', 'today_deposit_total_income'])
            ->find($userId);
    }

    private function isToday($user): bool
    {
        if (!$user || empty($user->today_deposit_stat_date)) {
            return false;
        }

        return date('Y-m-d', strtotime($user->today_deposit_stat_date)) === date('Y-m-d');
    }
}

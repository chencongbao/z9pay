...
return new class extends Migration
{
    ...
    {
        Schema::table('deposit_orders', function (Blueprint $table) {
            if (Schema::hasColumn('deposit_orders', 'user_agent4_commission')) {
                $table->dropColumn('user_agent4_commission');
            }
            if (Schema::hasColumn('deposit_orders', 'user_agent4_rate')) {
                $table->dropColumn('user_agent4_rate');
            }
            if (Schema::hasColumn('deposit_orders', 'user_agent4_id')) {
                $table->dropColumn('user_agent4_id');
            }
            if (Schema::hasColumn('deposit_orders', 'user_agent5_commission')) {
                $table->dropColumn('user_agent5_commission');
            }
            if (Schema::hasColumn('deposit_orders', 'user_agent5_rate')) {
                $table->dropColumn('user_agent5_rate');
            }
            if (Schema::hasColumn('deposit_orders', 'user_agent5_id')) {

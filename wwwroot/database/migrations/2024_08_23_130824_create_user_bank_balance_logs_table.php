...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('user_bank_balance_logs', function (Blueprint $table) {
            $table->id();
            $table->integer('user_bank_id')->index()->default(0)->comment('商户ID');
            $table->integer('user_id')->index()->default(0)->comment('金主ID');
            $table->integer('action_admin_id')->index()->default(0)->comment('金主ID');
    ...
    {
        Schema::dropIfExists('user_bank_balance_logs');
    }
};

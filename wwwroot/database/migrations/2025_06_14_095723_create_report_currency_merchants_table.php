...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('report_currency_merchants', function (Blueprint $table) {
            $table->id();


            $table->date("date_add")->nullable()->index()->comment("统计日期");
            $table->integer("mid")->default(0)->index()->comment("商户ID");
            $table->integer("cid")->default(0)->index()->comment("币种ID");
            ...
            $table->decimal("settlement_order_total_amount",30,2)->default(0)->comment("结算总跑量，成功单子总代收");
            $table->decimal("settlement_order_total_fee",30,2)->default(0)->comment("结算总手续费，成功单子总代收");
            $table->decimal("settlement_profit",30,2)->default(0)->comment("代付利润");
    ...
    {
        Schema::dropIfExists('report_currency_merchants');
    }
};

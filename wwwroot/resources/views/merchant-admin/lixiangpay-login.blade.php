<style>
    ...
    .login-lang-dropdown .dropdown-toggle {
        cursor: pointer;
    }

    .login-lang-dropdown .dropdown-menu {
        top: auto !important;
    ...
    .login-lang-dropdown .dropdown-toggle::after {
        margin-left: 6px;
        vertical-align: 2px;
    }

    .login-lang-dropdown .dropdown-menu::before {
        content: '';
        position: absolute;
        bottom: -6px;
        right: 18px;
    ...
    .login-lang-dropdown .dropdown-menu::after {
        content: '';
        position: absolute;
        bottom: -7px;
        right: 17px;
    ...
    .login-lang-dropdown .dropdown-menu .dropdown-item {
        display: block;
        padding: 7px 16px !important;
        line-height: 1.3 !important;
    }

    .login-lang-dropdown .dropdown-menu .dropdown-item + .dropdown-item {
        margin-top: 0;
    }
</style>

@php
    $adminLangKey = trim((string) config('merchant-admin.lang_key'));
    $adminTitle = $adminLangKey !== '' && \Illuminate\Support\Facades\Lang::has('admin.' . $adminLangKey) ? __('admin.' . $adminLangKey) : config('merchant-admin.title');
    $systemTitle = __('admin.merchant_admin_title');
@endphp
        ...
        <div class="card">
            ...
            <div class="card-body login-card-body shadow-100">
                <div class="login-logo mb-2">
                    <img src="{{asset('img/logo_lixiang.png')}}" class="logo" style="width: 50px;height: 50px;margin-right: 10px;"/><span>{{ $adminTitle }}</span>
                </div>
                ...
                <form id="login-form" method="POST" action="{{ admin_url('auth/login') }}">
                    ...
                    <fieldset class="form-label-group form-group position-relative has-icon-left">
                        <input
                            type="text"
                            class="form-control {{ $errors->has('coder') ? 'is-invalid' : '' }}"
                            name="coder"
                            id="coder"
                            placeholder="{{ trans('admin.coder') }}"
                            value="{{ old('coder') }}"
                            required
                            autofocus
                        >
                        ...
                        <label for="email">{{ trans('admin.coder') }}</label>

                        <div class="help-block with-errors"></div>
                        @if($errors->has('coder'))
                            <span class="invalid-feedback text-danger" role="alert">
                                            @foreach($errors->get('coder') as $message)
                    ...
                    <div class="form-group d-flex justify-content-between align-items-center remember">
                        <div class="text-left">
                            @if(intval(bob_admin_setting('merchant_system_login_remember_switch')))
                        ...
                        <div class="text-right">
                            <div class="login-lang-dropdown">
                                <div class="dropdown">
                                    <a class="dropdown-toggle btn btn-white btn-sm" href="javascript:;" data-toggle="dropdown" aria-expanded="false">
                                        {{ __('admin.'.config('app.locale')) }}
                                    </a>
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a href="javascript:;" class="dropdown-item js-switch-locale" data-locale="zh_CN">{{ __('admin.zh_CN') }}</a>
                                        <a href="javascript:;" class="dropdown-item js-switch-locale" data-locale="en">{{ __('admin.en') }}</a>
                                        <a href="javascript:;" class="dropdown-item js-switch-locale" data-locale="vi">{{ __('admin.vi') }}</a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    Dcat.ready(function () {
        ...
        let loginPassword = '';
        let loginRemember = 0;
        let baseUrl = "{{request()->getScheme()."://".config('merchant-admin.route.domain')}}/{{config('merchant-admin.route.prefix')}}";
        ...
        $('#mpanel1').slideVerify({
            ...
            beforeCheck:function(){
                ...
                var coder = $('#coder').val();
                if (coder == '' || name == '' || pass == '') {
                    Dcat.error(Dcat.lang.merchant_code_username_password_not_empty);
                    return false;
                }
                return true;
            },
        ...
        $('#submit-google-verify').on('click', function () {
            ...
            if (!loginCoder || !loginUsername || !loginPassword) {
            ...
            $.ajax({
                url: "/{{ config('merchant-admin.route.prefix') }}/auth/verify",
                type: 'POST',
                dataType: 'json',
                data: {
                    _token: "{{ csrf_token() }}",

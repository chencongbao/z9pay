<body
        class="dcat-admin-body sidebar-mini layout-fixed iframe-tab-page {{ $configData['horizontal_menu'] ? 'horizontal-menu' : '' }} {{ $configData['body_class']}} {{ $configData['sidebar_class'] }}
        {{ $configData['navbar_class'] === 'fixed-top' ? 'layout-navbar-fixed navbar-fixed-top' : '' }} ">
...
<script>
    (function () {
        ...
        // Dcat 水平菜单初始化会直接读取 .main-horizontal-sidebar.offset().top；
        // iframe-tab 父布局没有该节点时，状态开关刷新后会触发 undefined.top 异常。
        var guard = document.createElement('div');
        guard.className = 'main-horizontal-sidebar iframe-horizontal-menu-guard';
...
@php
    $iframeTabMd5JsPath = public_path('vendor/iframe-tab/js/md5.js');
    $iframeTabSwiperJsPath = public_path('vendor/iframe-tab/js/swiper.min.js');
    $iframeTabBaseJsPath = public_path('vendor/iframe-tab/js/base.js');
    $iframeTabExtendJsPath = public_path('vendor/iframe-tab/js/extend.js');
    $iframeTabMd5JsVersion = file_exists($iframeTabMd5JsPath) ? filemtime($iframeTabMd5JsPath) : time();
    $iframeTabSwiperJsVersion = file_exists($iframeTabSwiperJsPath) ? filemtime($iframeTabSwiperJsPath) : time();
    $iframeTabBaseJsVersion = file_exists($iframeTabBaseJsPath) ? filemtime($iframeTabBaseJsPath) : time();
    $iframeTabExtendJsVersion = file_exists($iframeTabExtendJsPath) ? filemtime($iframeTabExtendJsPath) : time();
@endphp
<script src="{{ asset('/vendor/iframe-tab/js/md5.js') }}?v={{ $iframeTabMd5JsVersion }}"></script>
<script src="{{ asset('/vendor/iframe-tab/js/swiper.min.js') }}?v={{ $iframeTabSwiperJsVersion }}"></script>
<script src="{{ asset('/vendor/iframe-tab/js/base.js') }}?v={{ $iframeTabBaseJsVersion }}"></script>
<script src="{{ asset('/vendor/iframe-tab/js/extend.js') }}?v={{ $iframeTabExtendJsVersion }}"></script>

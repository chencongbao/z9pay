...
;(function($, window, document,undefined) {
	...
	function captchaRequestError(xhr) {
		var status = xhr && xhr.status ? xhr.status : 0;
		var message = Dcat.lang.captcha_network_error || Dcat.lang.captcha_fail;

		if (status === 429) {
			var retryAfter = xhr && xhr.getResponseHeader ? xhr.getResponseHeader('Retry-After') : null;
			message = (Dcat.lang.captcha_rate_limited || message).replace(':seconds', retryAfter || '1');
		}

		return {
			error: true,
			repCode: String(status || 'NETWORK_ERROR'),
			repData: null,
			repMsg: message,
			success: false
		};
	...
	function rejectCaptchaRequest(reject, xhr) {
		if (typeof reject === 'function') {
			reject(captchaRequestError(xhr));
		}
	}
    ...
    function getPictrue(data,baseUrl,resolve,reject){
		$.ajax({
			...
			success:function(res){
                $("#mpanel1").css("display","block");
				resolve(res)
			},
			error: function(xhr) {
				$("#mpanel1").css("display","block");
				rejectCaptchaRequest(reject, xhr);
			}
		})
	}
	//验证图片check事件
    ...
    var Slide = function(ele, opt) {
        ...
        this.defaults = {
        	...
        	vOffset: 5,
            vSpace : 5,
            explain : Dcat.lang.captcha_swipe_right,
            imgSize : {
	        	width: '310px',
	        	height: '155px',
			},
        	...
        	if(this.status  && this.isEnd == false) {
				...
				checkPictrue(data,this.options.baseUrl,function(res){
					...
					if (res.repCode=="0000") {
						...
						_this.htmlDoms.icon.addClass('icon1-check');
						//提示框
						_this.htmlDoms.tips.addClass('suc-bg').removeClass('err-bg')
						// _this.htmlDoms.tips.css({"display":"block",animation:"move 1s cubic-bezier(0, 0, 0.39, 1.01)"});
						_this.htmlDoms.tips.animate({"bottom":"0px"});
						_this.htmlDoms.tips.text(((_this.endMovetime-_this.startMoveTime)/1000).toFixed(2) + 's'+Dcat.lang.captcha_success);
						_this.isEnd = true;
						setTimeout(function(){
							_this.$element.find(".mask").css("display","none");
						...
						// 	// _this.htmlDoms.tips.css({"display":"none",animation:"none"});
						// },1300)
						_this.options.error(_this, res);
					}
				}, function (res) {
					_this.htmlDoms.move_block.css('background-color', '#d9534f');
					_this.htmlDoms.left_bar.css('border-color', '#d9534f');
					_this.htmlDoms.icon.css('color', '#fff').removeClass('icon-right').addClass('icon-close');
					_this.htmlDoms.tips.addClass('err-bg').removeClass('suc-bg').animate({"bottom":"0px"}).text(res.repMsg);
					_this.options.error(_this, res);
				})
	            this.status = false;
        	}
        ...
        refresh: function() {
			...
			getPictrue({captchaType:"blockPuzzle", clientUid: localStorage.getItem('slider'), ts: Date.now()},this.options.baseUrl,function (res) {
				if (res.repCode=="0000") {
					...
					_this.$element.find(".backImg")[0].src = '/vendor/captcha/images/default.jpg'
					...
					setTimeout(function(){
						...
						checkPictrue(data, _this.options.baseUrl,function(res){
							if (res.repCode=="0000") {
								...
								_this.$element.find('.verify-msg').text(Dcat.lang.captcha_success);
								...
								_this.$element.find('.verify-msg').text(Dcat.lang.captcha_fail);
								setTimeout(function () {
        ...
        refresh: function() {
			...
			getPictrue({captchaType:"clickWord", clientUid: localStorage.getItem('point'), ts: Date.now()},_this.options.baseUrl,function(res){
				if (res.repCode=="0000") {
					...
					_this.htmlDoms.back_img[0].src = '/vendor/captcha/images/default.jpg';
					_this.$element.find('.verify-msg').text(res.repMsg);
				}
			}, function (res) {
				_this.htmlDoms.back_img[0].src = '/vendor/captcha/images/default.jpg';
